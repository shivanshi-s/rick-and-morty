...
const Card = ({character}) => {
    return <li className="border-2 border-black p-4 w-64 rounder-md">
        <h1 className="text-xl font-bold"><span>{getCharacterStatus(character.status)}</span>    {character.name}</h1>
        <img src={character.image} alt={character.name} className="my-3" />
        <span className="bg-teal-200 p-1 text-sm rounded-sm">{character.species}</span>
    </li>
}
